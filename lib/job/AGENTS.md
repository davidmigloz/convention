...
5. **Concurrent-insert race convergence**: if `Insert` fails with `errors.Is(err, convDB.ErrDuplicateID)` — a peer replica inserted the same `jid` between this call's `SelectByID` and its own `Insert` — `Register` re-`SelectByID`s. A found row is reconciled via `applyPersistedJob` (the same helper the "DB row exists but no in-memory entry" path below uses), exactly as if this had been an ordinary post-restart `Register`. A `nil` result (the racer's row already vanished — concurrent `Unregister` churn) triggers a **bounded, one-time retry** of the `Insert`: two sequential blocks, deliberately not a loop and not `mutateLoop`-style backoff — this is a one-time startup race, not a hot write path. A second duplicate on the retry gives up with a plain (non-sentinel) error naming the register/unregister churn — non-sentinel deliberately, so persistent churn cannot be mistaken for the ordinary converged race, but it carries the underlying duplicate-key error as its message text (`: %v`) so the cause still reaches the operator.

   **Outcome worth knowing**: if a peer's `Unregister` is what made the racer's row vanish, the bounded retry re-creates the job. That is the same end state the interleaving would have produced without the race (this `Register` is the caller's explicit intent, and it ran after that `Unregister`), and it cannot happen between two same-process callers — both hold `mut` for the whole of `Register`/`Unregister`. A caller that needs an unregistration to stick against a concurrently-booting replica must stop calling `Register` for that job, not rely on ordering.
6. **Persist-before-memory:** an interval change is written to the DB **first**; only on success is the in-memory map mutated. The scheduler goroutine is already running, so an un-persisted memory change that the next `syncJobsFromDB` reverts would flap. On `Update` error, memory is left untouched and the error returns. The same ordering holds for the insert-branch race above: `Insert`/retry-`Insert` (persist) always precedes the `jobs[tenant][jid]` assignment (memory).
7. **Anchor-change escape hatch:** a *same-interval* fire-time change (daily 03:00→04:00, weekly Monday→Tuesday — same `RepeatEvery`) is **not** auto-re-anchored (to preserve no-clock-reset on redeploy). To change a fire time, do a deliberate one-time `Unregister` + `Register` (or use a new job ID). **Do not** reintroduce a per-deploy unregister/register dance for this.
8. Sends a non-blocking signal on `wakeUp` (via `wake()`) to wake the background loop on every success path (initial insert, race-converged, retried insert, and the pre-existing "DB row exists" path)

**Test seam**: `registerInsertRaceHookForTest`, when non-nil, is invoked immediately before each `Insert` attempt Register makes (both the initial attempt and the bounded retry) — lets tests deterministically inject a peer replica's concurrent Insert/Delete at the exact point a real race would land. See `export_test.go`'s `SetRegisterInsertRaceHookForTest`. Nil in production.
...
| `Insert` | `Register` | Persist new job; a runtime-table PK collision surfaces as `convDB.ErrDuplicateID`, which `Register` handles by re-`SelectByID`-ing and either converging (`applyPersistedJob`) or retrying the `Insert` exactly once |
